...
def query_records(format,query,display_fields=None):
    ...
    try:
        ...
        return "%(error)s" % locals()



    if format in ['python','json']:
        json_data = {'record_list': encode_model(record_list), 'query': query, 'display_fields': display_fields, 'count': record_list_count}
        print json_data
        return jsonify( json_data )
...
def query_records_with_display_fields(format,query, display_fields):
    debug("qrwdf(format=%(format)s query=%(query)s display_fields=%(display_fields)s" % locals())
